...
| Firebase | This JavaScript module handles data storing as well as user authentication. |
...
| DiscussionPage | A primary component of the application which allows for stakeholders to engage in discussions with one another regarding ChatGPT. It utilizes Firebase’s database to store all the posts.|
...
| HomePage | This component is the home page to the website. The App component will display this component first when a stakeholder navigates to the website. |
| LikeDislike | This is a subcomponent of the `DiscussionPage` component. Its primary job is being the like and dislike buttons of each discussion post. | 
...
| Tweets | This component shows relevant Tweets from many Artificial Inteligence researchers. |
...
1. Duplicated Code - The handleLikePost and handleDislikePost have similar functionality. The only difference between them is one handles likes while the other handles the dislikes.
1. Comments - Throughout the whole codebase, there are many comments scattered around that either try to explain the code or are just unnecessary.
...
1. Click the `q` key or `Control + C` to end the test in terminal. 

Although I have many tests which span over many components, I am focusing solely on the `DiscussionPage` component. Below is a brief description of each test I conducted and what they do. 

#### Test Discussion Posts Functionality 

| Test | What does it do? |
| :--- | :--------------- | 
| Renders Empty Discussion Page | This tests if the component can be rendered when App calls it. |
| Renders Discussion Post | This tests if a discussion post can be rendered correctly. |
| Renders Disscusion Post and Click Like (2x) | This tests if a discussion post can be rendered correctly and if you can click the like button and see the value update. |
| Renders Disscusion Post and Click Dislike (2x) | This tests if a discussion post can be rendered correctly and if you can click the dislike button and see the value update |
| Renders Disscusion Post and Click Both Like/Dislike | This tests if a discussion post can be rendered correctly and if you can click both the like and dislike button and see the values update. |
...
| Like/Dislike Button Interactivity | This test simulates simultaneous clicking of the like and dislike buttons to ensure that the values increament/decrement correctly |

*Table 3: DiscussionPage Component Automated Tests*

All of the tests in *Table 3* pass giving me 100% test coverage within the `DiscussionPage` component. Below in *Image 3*, is the current test coverage report of the entire application. In the real world, it is advised to have tests which cover all the components within your application. Although I was very close to achieving a 100% coverage score for all categories, I was unable to finish testing for the `App`, `ProfilePage`, `SignInPage`, and `Tweets` components. Even though my `Tweets` component is showing 100% coverage, I was unable to verify that my component behaves the way it should. I needed to comment out this test along with an authentication test due to it causing my GitHub classroom tester to fail. In the end, I was able to have 27 automated tests that all pass and function correctly (see *Image 4* below)!
...
1. Code is too large in class - I refactored the RenderAllPost function by creating a separate function which solely renders a single post. The refactoring also included adjustments in variable naming to further improve readability of the function.
...
1. Duplicated Code - To address fix the redundancy between `handleLikePost` and `handleDislikePost`, I created a new variable named `handleReaction` which still accepts a post as a parameter. Its additional parameter is whether the post has recently received a like or dislike reaction. This will assist `handleReaction` in setting the appropriate state.
1. Comments - I went through all the components double checking the comments. I either refactored the code to make it more readable or just erased the unnecessary comment. 
